...
export default class MedableHttpClient extends HttpClient {
  constructor(client) {
    super(client);
  }
  ...
  async createJWT(apiKey, username) {
    const tokenScript =
      `const Account = org.objects.account; const account_id= Account.find({email:"${username}"}).skipAcl().grant(4).next()._id; return Account.createAuthToken('${apiKey}', account_id, { scope:['*'], skipAcl: true, grant: consts.accessLevels.read})`;
